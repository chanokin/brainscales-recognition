...
kenyon_parameters['i_offset'] = np.random.normal(i_offset, sigma_i_offset, size=args.nKC)  # mA
kenyon_parameters['v_thresh'] = np.random.normal(v_thresh, sigma_v_thresh, size=args.nKC)
# kenyon_parameters['v_reset'] = -65.0
# kenyon_parameters['cm'] = 0.1

inh_kenyon_parameters = base_params.copy()
inh_kenyon_parameters['tau_syn_E'] = 30.0  # ms
inh_kenyon_parameters['tau_syn_I'] = 1.0  # ms
inh_kenyon_parameters['tau_refrac'] = 1.0  # ms
inh_kenyon_parameters['tau_m'] = 5.0  # ms
inh_kenyon_parameters['i_offset'] = np.random.normal(i_offset, sigma_i_offset, size=args.nKC)  # mA
inh_kenyon_parameters['v_thresh'] = np.random.normal(v_thresh, sigma_v_thresh, size=args.nKC)
# inh_kenyon_parameters['v_reset'] = -65.0
# inh_kenyon_parameters['cm'] = 0.1
...
decision_parameters['i_offset'] = np.random.normal(i_offset, sigma_i_offset*4, size=args.nDN)  # mA
decision_parameters['v_thresh'] = np.random.normal(v_thresh, sigma_v_thresh*4, size=args.nDN)

inh_decision_parameters = base_params.copy()
inh_decision_parameters['tau_syn_E'] = 30.0  # ms
inh_decision_parameters['tau_syn_I'] = 1.0  # ms
inh_decision_parameters['tau_refrac'] = 1.0
inh_decision_parameters['tau_m'] = 5.0
inh_decision_parameters['i_offset'] = np.random.normal(i_offset, sigma_i_offset*4, size=args.nDN)  # mA
inh_decision_parameters['v_thresh'] = np.random.normal(v_thresh, sigma_v_thresh*4, size=args.nDN)
...
# NoisyCurrentSource(mean=1.5, stdev=1.0, start=50.0, stop=450.0, dt=1.0)
populations = {
    ...
    # 'noise_kenyon': pynnx.Pop(args.nKC, 'NoisyCurrentSource',
    #                     {'mean': np.random.normal(i_offset, sigma_i_offset, size=args.nKC),
    #                      'stdev': sigma_i_offset,},
    #                     label='noise Kenyon Cell',
    #                     hicann_id=hicanns['kenyon']),
    'inh_kenyon': pynnx.Pop(args.nKC, neuron_class,
                            inh_kenyon_parameters, label='Inh Kenyon Cell',
                            hicann_id=hicanns['kenyon']),
    'decision': pynnx.Pop(args.nDN, neuron_class,
                          decision_parameters, label='Decision Neurons',
                          hicann_id=hicanns['decision']),
    # 'noise_decision': pynnx.Pop(args.nDN, 'NoisyCurrentSource',
    #                         {'mean': np.random.normal(i_offset, sigma_i_offset, size=args.nKC),
    #                          'stdev': sigma_i_offset, },
    #                         label='Decision Neurons',
    #                         hicann_id=hicanns['decision']),
    'inh_decision': pynnx.Pop(args.nDN, neuron_class,
                              inh_decision_parameters, label='Inh Decision Neurons',
                              hicann_id=hicanns['decision']),
...
# populations['decision'].initialize(i_offset=np.random.uniform(i_offset, sigma_i_offset, size=args.nDN),)

pynnx.set_recording(populations['kenyon'], 'spikes')
populations['kenyon'].initialize(v_thresh=np.random.normal(v_thresh, sigma_v_thresh, size=args.nKC),)
# populations['kenyon'].initialize(i_offset=np.random.uniform(i_offset, sigma_i_offset, size=args.nKC),)
...
if neuron_class == 'IF_cond_exp':
    static_w = {
        ...
        'KC to DN': W2S * (0.1 * (2500.0 / float(args.nKC))),
        'DN to DN': W2S * (1.0 * (100.0 / float(args.nDN))),

        'DN to FB': W2S * (1.1 * (100.0 / float(args.nDN))),
        'FB to DN': W2S * (6.0 * (100.0 / float(args.nDN))),
        'TK to FB': W2S * (1.1 * (100.0 / float(args.nDN))),
        ...
        'SRC to INH_FB': W2S * 0.4,
        # 'INH_FB to SRC': W2S * 5.0,
        'INH_FB to SRC': W2S * 0.0000000000000000001,
...
projections = {
    ...
    'KC to IKC': pynnx.Proj(populations['kenyon'], populations['inh_kenyon'],
                           'OneToOneConnector', weights=static_w['SRC to INH_FB'],
                            delays=timestep,
                            target='excitatory', label='KC to IKC',),

    'IKC to KC': pynnx.Proj(populations['inh_kenyon'], populations['kenyon'],
                           'OneToOneConnector', weights=static_w['INH_FB to SRC'],
                            delays=timestep,
                            target='inhibitory', label='IKC to KC',),
...
# populations['kenyon'].inject(populations['noise_kenyon']),
# populations['decision'].inject(populations['noise_decision'])
...
if record_all:
    ...
    inh_dec_spikes = pynnx.get_record(populations['inh_decision'], 'spikes')
    inh_kny_spikes = pynnx.get_record(populations['inh_kenyon'], 'spikes')
...
fname = 'mbody-experiment-thresholds.npz'
np.savez_compressed(fname, args=args, sim_time=sim_time,
                    ...
                    inh_kny_spikes=inh_kny_spikes,
                    inh_dec_spikes=inh_dec_spikes,
                    )
