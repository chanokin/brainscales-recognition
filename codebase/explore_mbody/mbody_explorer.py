...
def eval_one_min(trajectory):
    ...
    populations = {
        label: {'type': neuron_types[label],
                'size': pop_sizes[label],
                'params': neuron_params[label],
                'record': ['spikes',]}
        for label in neuron_types
    }
    ...
    projections = {
        'antenna':{
            'kenyon': {
              ...
              'weights': rand_w['AL to KC'], 'delays': 1.0,
              'conn_params': {'p_connect': prob_antenna_to_kenyon},
              'label': 'AL to KC',
            },
            'horn': {
                'conn': 'FromListConnector',
                'conn_params': {'conn_list': gain_list},
                'label': 'AL to LH',
            },
        },
        ...
        'horn': {
            'kenyon': {
                'conn': 'AllToAllConnector',
                'weights': static_w['LH to KC'], 'delays': 1.0,
                'target': 'inhibitory', 'label': 'LH to KC'
            }
        },
        ...
        'decision':{
            'decision': {
                'conn': 'AllToAllConnector',
                'weights': static_w['DN to DN'], 'delays': 1.0,
